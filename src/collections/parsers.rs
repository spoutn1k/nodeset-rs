...
type PResult<'k, T> = Result<T, ErrMode<CustomError<&'k str>>>;
...
impl<'a, 'k> Parser<'a> {
    ...
    pub fn parse<T>(self, i: &'k str) -> Result<NodeSet<T>, NodeSetParseError>
    where
        T: IdRange + PartialEq + Clone + fmt::Display + fmt::Debug,
    {
        let mut ns = self
            .expr()
            .parse(i)
            .map_err(|e| NodeSetParseError::from(e.into_inner()))?;

        ns.fold();
        Ok(ns)
    }

    fn expr<T>(self) -> impl 'a + FnMut(&mut &'k str) -> PResult<'k, NodeSet<T>>
    where
        T: IdRange + PartialEq + Clone + fmt::Display + fmt::Debug,
    {
        move |i: &mut &str| {
    ...
    fn term<T>(self) -> impl 'a + FnMut(&mut &'k str) -> PResult<'k, NodeSet<T>>
    where
        T: IdRange + PartialEq + Clone + fmt::Display + fmt::Debug,
    {
        move |input: &mut &str| {
    ...
    fn op(i: &mut &'k str) -> PResult<'k, char> {
        alt((
    ...
    fn group_or_nodeset<T>(self) -> impl 'a + FnMut(&mut &'k str) -> PResult<'k, NodeSet<T>>
    where
        T: IdRange + PartialEq + Clone + fmt::Display + fmt::Debug,
    {
    ...
    fn nodeset_or_rangeset<T>(i: &mut &'k str) -> PResult<'k, NodeSet<T>>
    where
        T: IdRange + PartialEq + Clone + fmt::Display + fmt::Debug,
    {
    ...
    fn rangeset<T>(i: &mut &'k str) -> PResult<'k, NodeSet<T>>
    where
        T: IdRange + PartialEq + Clone + fmt::Display + fmt::Debug,
    {
        (
            alt((Self::id_range_bracketed_affix, Self::id_range_step_rangeset)),
            peek(alt((alt((",", "&", "!", "^", "(", ")")), multispace1, eof))),
        )
            .try_map(|(idrs, _)| {
    ...
    fn nodeset<T>(i: &mut &'k str) -> PResult<'k, NodeSet<T>>
    where
        T: IdRange + PartialEq + Clone + fmt::Display + fmt::Debug,
    {
    ...
    fn sourceset<T>(i: &mut &'k str) -> PResult<'k, NodeSet<T>>
    where
        T: IdRange + PartialEq + Clone + fmt::Display + fmt::Debug,
    {
    ...
    fn set<T>(source: bool) -> impl 'a + FnMut(&mut &'k str) -> PResult<'k, NodeSet<T>>
    where
        T: IdRange + PartialEq + Clone + fmt::Display + fmt::Debug,
    {
        move |i: &mut &str| {
    ...
    #[auto_enum]
    fn group<T>(self) -> impl 'a + FnMut(&mut &'k str) -> PResult<'k, NodeSet<T>>
    where
        T: IdRange + PartialEq + Clone + fmt::Display + fmt::Debug,
    {
        move |i: &mut &str| {
            preceded(
                ...
                alt((
                    ...
                    Self::nodeset_or_rangeset.map(|s: NodeSet<T>| (None, Some(s))),
                )),
            )
            .map(
            ...
            .map(|r| r.map_err(|e| ErrMode::Cut(CustomError::NodeSetError(e))))
            .parse_next(i)?
        }
    }
    ...
    fn group_with_source<T>(
        self,
    ) -> impl FnMut(&mut &'k str) -> PResult<'k, (Option<NodeSet<T>>, Option<NodeSet<T>>)>
    where
        T: IdRange + PartialEq + Clone + fmt::Display + fmt::Debug,
    {
        move |i: &mut &str| {
    ...
    fn node_component(i: &mut &'k str) -> PResult<'k, &'k str> {
        take_while(1.., is_nodeset_char).parse_next(i)
    }

    fn source_component(i: &mut &'k str) -> PResult<'k, &'k str> {
    ...
    #[allow(clippy::type_complexity)]
    fn id_standalone(i: &mut &'k str) -> PResult<'k, IdRangeComponent> {
        digit1
    ...
    fn id_range_step_rangeset(i: &mut &'k str) -> PResult<'k, IdRangeComponent> {
        Self::id_range_step
...
impl<I: Stream + Clone> ParserError<I> for CustomError<I> {
    fn from_error_kind(input: &I, kind: ErrorKind) -> Self {
        CustomError::ParserErrorKind(input.clone(), kind)
    }
...
mod tests {
    ...
    fn test_node_component() {
        ...
        assert_eq!(
            Parser::node_component(&mut "http://a1a").unwrap(),
            "http://a"
        );
    }
    ...
    fn test_source_component() {
        ...
        assert_eq!(Parser::source_component(&mut "http://a1a").unwrap(), "http");
    }

    fn test_component<'a>(parser: impl Fn(&mut &'a str) -> PResult<'a, &'a str>) {
